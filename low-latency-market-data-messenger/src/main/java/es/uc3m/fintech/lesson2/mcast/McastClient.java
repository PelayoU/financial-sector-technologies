...
public class McastClient {
    ...
    private static void runClient(String multicastAddress, int port, int durationSeconds)
        ...
        clientSocket.joinGroup(new InetSocketAddress(address, port), null);
        ...
        try {
            while (System.currentTimeMillis() < endTime) {
                ...
                MarketMessage message = receiveMessage(receiveBuffer, clientSocket);
    ...
    private static MarketMessage receiveMessage(
        ...
        // message)
        return MarketMessage.fromBinary(
                java.util.Arrays.copyOf(msgPacket.getData(), msgPacket.getLength())
        );
