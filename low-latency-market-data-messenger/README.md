...
The project is end-to-end inspectable on a single machine: start server and client(s) in separate JVMs, capture traffic with Wireshark, and confirm the frames on the wire match the binary protocol.
...
```bash
# TCP: start the server first, then the client in a second terminal
mvn exec:java@tcp-server
mvn exec:java@tcp-client

# Multicast: start the sender first, then one or more receivers
...
Implementation follows the exercises and protocol description from **Lección 2 — Mensajería de Baja Latencia** of the Master in Financial Sector Technologies (UC3M), focused on the socket-level fundamentals every market-data gateway is built on top of.
